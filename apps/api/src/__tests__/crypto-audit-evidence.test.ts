...
describe("cryptographic audit evidence", () => {
  it("changes the SHA-256 hash when artifact bytes change", () => {
    expect(sha256Bytes(Buffer.from("file-v1"))).not.toBe(
      sha256Bytes(Buffer.from("file-v2")),
    );
  });

  it("rejects tampered payloads and the wrong public key", () => {
    const payload = canonicalEvidence({
      artifactHash: "a",
      action: "approved",
    });
    ...
    expect(
      verifyEvidence(
        canonicalEvidence({ artifactHash: "tampered", action: "approved" }),
        signed.signature,
      ),
    ).toBe(false);
  ...
  it("detects a broken audit hash chain", () => {
    ...
    expect(verifyAuditChain([{ ...record, artifactHash: "tampered" }])).toBe(
      false,
    );
  });
});
