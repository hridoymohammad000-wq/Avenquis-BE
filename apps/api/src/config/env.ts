...
export function parseEnv(source: NodeJS.ProcessEnv): ApiEnv {
  ...
  const values = {
    ...
    JWT_SECRET:
      source.JWT_SECRET ??
      (nodeEnv === "production" ? undefined : DEV_JWT_SECRET),
  ...
  if (
    result.success &&
    ((result.data.SIGNING_PRIVATE_KEY && !result.data.SIGNING_PUBLIC_KEY) ||
      (!result.data.SIGNING_PRIVATE_KEY && result.data.SIGNING_PUBLIC_KEY))
  ) {
    throw new Error(
      "Invalid signing configuration: both signing keys are required together",
    );
  }
  if (
    !result.success ||
