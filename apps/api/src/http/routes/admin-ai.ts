...
adminAiRouter.patch("/agents/:id", ...write, async (req, res, next) => { try { const parsed = z.object({ enabled: z.boolean().optional(), autonomyLevel: z.enum(["L0", "L1", "L2", "L3"]).optional() }).refine((v) => Object.keys(v).length > 0).parse(req.body); res.json({ success: true, data: await AiAdminService.updateAgent(req.params.id, parsed, { userId: req.user!.id, role: req.platformRoles![0] }) }); } catch (e) { next(e); } });
