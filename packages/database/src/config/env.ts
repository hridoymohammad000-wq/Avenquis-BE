...
export function parseDatabaseEnv(source: NodeJS.ProcessEnv): DatabaseEnv {
  ...
  const databaseUrl = source.DATABASE_URL ?? (nodeEnv === "production" ? undefined : DEV_DATABASE_URL);
  const result = envSchema.safeParse({
    ...source,
    NODE_ENV: nodeEnv,
    DATABASE_URL: databaseUrl,
  });
  if (!result.success || (nodeEnv === "production" && databaseUrl && /USER|PASSWORD|HOST|DATABASE/.test(databaseUrl))) {
    throw new Error("Invalid database configuration: DATABASE_URL is required and must be a valid PostgreSQL URL");
  }
  return result.data;
}
